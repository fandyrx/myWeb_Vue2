<template>
  <div class="controler">
    <div class="left">
      <img
        :class="{ discAnimation: isPlay }"
        v-lazy="playInfo[playIndex].al.picUrl"
        alt="歌曲封面"
      />
      ...
      <div class="auth">
        ...
        <div :class="{ marqueeWrap: true, move: isPlay }">
          <div :class="{ singer: true, textRoll: isPlay }">
            <span
              v-for="(singer, index) in playInfo[playIndex].ar"
              :key="index"
            >
              {{ singer.name }}
            </span>
    ...
    <div class="right">
      <div class="voice-control">
        <el-slider class="slider" v-model="voice" vertical v-show="voiceShow">
        </el-slider>
...
export default {
  ...
  methods: {
    ...
    changeShowTime(value) {
      ...
      this.currentTime = moment(value * this.$refs.audio.duration * 10).format(
        "mm:ss"
      );

      if (this.currentTime == "Invalid date") this.currentTime = "00:00"; //初始化，切换bug
    },
